...
An example deployment, where `jacocoagent.jar` is included in the container image and the JaCoCo server 
running behind the hostname `jacoco-server.default.svc.cluster.local`:
```yaml
apiVersion: v1
kind: Pod
metadata:
